<script lang="ts">
  ...
  function initTables(tab: number[], permTable: Uint8Array, gradTable: Float32Array) {
  ...
  const noise = create([
    ...
    74, 155, 33, 15, 219, 130, 226, 202, 83, 236, 42, 172, 165, 218, 55, 222, 46, 107, 98, 154, 109,
    67, 196, 178, 127, 158, 13, 243, 65, 79, 166, 248, 25, 224, 115, 80, 68, 51, 184, 128, 232, 208,
    151, 122, 26, 212, 105, 43, 179, 213, 235, 148, 146, 89, 14, 195, 28, 78, 112, 76, 250, 47, 24,
    251, 140, 108, 186, 190, 228, 170, 183, 139, 39, 188, 244, 246, 132, 48, 119, 144, 180, 138,
    134, 193, 82, 182, 120, 121, 86, 220, 209, 3, 91, 241, 149, 85, 205, 150, 113, 216, 31, 100, 41,
    164, 177, 214, 153, 231, 38, 71, 185, 174, 97, 201, 29, 95, 7, 92, 54, 254, 191, 118, 34, 221,
    131, 11, 163, 99, 234, 81, 227, 147, 156, 176, 17, 142, 69, 12, 110, 62, 27, 255, 0, 194, 59,
    116, 242, 252, 19, 21, 187, 53, 207, 129, 64, 135, 61, 40, 167, 237, 102, 223, 106, 159, 197,
  ...
  function attachNoise(node: HTMLCanvasElement) {
    ...
    const bgColor = computedStyle.getPropertyValue('background-color').slice(4, -1);
    const fgColor = computedStyle.getPropertyValue('color').slice(4, -1);
    const [bg1, bg2, bg3] = bgColor.split(', ').map(Number);
    const [fg1, fg2, fg3] = fgColor.split(', ').map(Number);
    ...
    const go = () => {
      ...
      const chunkSize = Math.max(1, Math.floor(diagonal / 100));
      const scaling = 0.005;
      ...
      for (let x = 0; x < numChunksX; x++) {
        let noiseTop = noise(x * chunkSize * scaling, -1 * chunkSize * scaling, z);
        for (let y = 0; y < numChunksY; y++) {
          const val = noise(x * chunkSize * scaling, y * chunkSize * scaling, z);
          ...
          for (let fy = 0; fy < maxFy; fy++) {
            const valFactor = fy / chunkSize;
            let factor = noiseTop + (val - noiseTop) * valFactor;
            factor += 0.5;
            const color1 = bg1 + (fg1 - bg1) * factor;
            const color2 = bg2 + (fg2 - bg2) * factor;
            const color3 = bg3 + (fg3 - bg3) * factor;
            for (let fx = 0; fx < maxFx; fx++) {
              const i = (y * chunkSize + fy) * 4 * node.width + (x * chunkSize + fx) * 4;
              pixels.data[i] = color1;
              pixels.data[i + 1] = color2;
              pixels.data[i + 2] = color3;
              pixels.data[i + 3] = 255;
            }
          }
    ...
    const render = (now: number) => {
      let mouseAccel = Math.sqrt((mouseX - lastMouseX) ** 2 + (mouseY - lastMouseY) ** 2);
      mouseAccel *= 0.003;
      mouseAccel = Math.min(mouseAccel, 0.25);
...
<canvas
  {@attach attachNoise}
  style={`position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; background-color: var(--m3c-surface); color: var(--m3c-primary-container-subtle); clip-path:inset(calc(50% * var(--stillness)) round calc(4rem * var(--stillness)))`}
></canvas>
